...
設定值
    ...
    - 目錄
        ...
        - 共用套件目錄: package
        - 執行檔目錄: .
        - 功能規格目錄: 20.doc/48.spec/frontend/
...
#### 
  由 參考系統 1對1 將所對應的 源碼 (原為 vue) 轉成 react 
  進行以下前置功能
     -. 分析 參考系統 更新
    ...
    -. 依 轉換檢查表 進行所有檔案一對一的對應，如為 網頁使用的 view or js ，則不轉(進行標記), 如為前端或後端的 程式，轉成 react
    -. 轉換前，先分析 參考系統 所使用的 framework, 並在 react 中選出相對應的，並將 選擇結果，記在
        {功能規格目錄}/50.Framework.md
產生 工作用 shell (建立在 frontend 的目錄下)
