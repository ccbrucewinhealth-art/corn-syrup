// checklist translated from TwoFADialog.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TwoFaDialogComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for TwoFaDialogComponent {
    fn default() -> Self {
        logging::debug("ui.two_fa_dialog", "default", "init component with translated field schema");
        Self { source_path:"TwoFADialog.vue", title:"2FA 對話框".to_string(), route:"/two-fa-dialog".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl TwoFaDialogComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.two_fa_dialog", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.two_fa_dialog", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.two_fa_dialog", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.two_fa_dialog", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.two_fa_dialog", "default_fields", "build component-specific schema");
    vec![
        ...
        UiField { name: "secret".to_string(), label: "TOTP Secret".to_string(), required: true, value: None, visible: true, field_type: "password".to_string(), sensitive: true },
        UiField { name: "token".to_string(), label: "Verification Code".to_string(), required: true, value: None, visible: true, field_type: "otp".to_string(), sensitive: true },
        UiField { name: "recovery_code".to_string(), label: "Recovery Code".to_string(), required: false, value: None, visible: true, field_type: "password".to_string(), sensitive: true }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.two_fa_dialog", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&TwoFaDialogComponent)->Vec<(String,String)>{
    logging::debug("ui.two_fa_dialog", "render_model_compat", "compat render");
