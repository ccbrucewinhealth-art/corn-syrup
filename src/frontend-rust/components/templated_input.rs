// checklist translated from TemplatedInput.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TemplatedInputComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for TemplatedInputComponent {
    fn default() -> Self {
        logging::debug("ui.templated_input", "default", "init component with translated field schema");
        Self { source_path:"TemplatedInput.vue", title:"範本輸入".to_string(), route:"/templated-input".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl TemplatedInputComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.templated_input", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.templated_input", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.templated_input", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.templated_input", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.templated_input", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.templated_input", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&TemplatedInputComponent)->Vec<(String,String)>{
    logging::debug("ui.templated_input", "render_model_compat", "compat render");
