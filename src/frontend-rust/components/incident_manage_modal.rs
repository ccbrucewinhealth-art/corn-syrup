// checklist translated from IncidentManageModal.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IncidentManageModalComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for IncidentManageModalComponent {
    fn default() -> Self {
        logging::debug("ui.incident_manage_modal", "default", "init component with translated field schema");
        Self { source_path:"IncidentManageModal.vue", title:"事件管理彈窗".to_string(), route:"/incident-manage-modal".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl IncidentManageModalComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.incident_manage_modal", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.incident_manage_modal", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.incident_manage_modal", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.incident_manage_modal", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.incident_manage_modal", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "title".to_string(), label: "Incident Title".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "status".to_string(), label: "Status".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "content".to_string(), label: "Content".to_string(), required: true, value: None, visible: true, field_type: "textarea".to_string(), sensitive: false },
        UiField { name: "pinned".to_string(), label: "Pinned".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.incident_manage_modal", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&IncidentManageModalComponent)->Vec<(String,String)>{
    logging::debug("ui.incident_manage_modal", "render_model_compat", "compat render");
