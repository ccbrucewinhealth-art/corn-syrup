// checklist translated from HeartbeatBar.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HeartbeatBarComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for HeartbeatBarComponent {
    fn default() -> Self {
        logging::debug("ui.heartbeat_bar", "default", "init component with translated field schema");
        Self { source_path:"HeartbeatBar.vue", title:"心跳條".to_string(), route:"/heartbeat-bar".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl HeartbeatBarComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.heartbeat_bar", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.heartbeat_bar", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.heartbeat_bar", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.heartbeat_bar", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.heartbeat_bar", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "monitor_id".to_string(), label: "Monitor ID".to_string(), required: true, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "range".to_string(), label: "Range".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "samples".to_string(), label: "Samples".to_string(), required: false, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "timezone".to_string(), label: "Timezone".to_string(), required: false, value: None, visible: true, field_type: "select".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.heartbeat_bar", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&HeartbeatBarComponent)->Vec<(String,String)>{
    logging::debug("ui.heartbeat_bar", "render_model_compat", "compat render");
