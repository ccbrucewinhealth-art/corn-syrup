// checklist translated from MaintenanceTime.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MaintenanceTimeComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for MaintenanceTimeComponent {
    fn default() -> Self {
        logging::debug("ui.maintenance_time", "default", "init component with translated field schema");
        Self { source_path:"MaintenanceTime.vue", title:"維護時間".to_string(), route:"/maintenance-time".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl MaintenanceTimeComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.maintenance_time", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.maintenance_time", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.maintenance_time", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.maintenance_time", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.maintenance_time", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "title".to_string(), label: "Maintenance Title".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "strategy".to_string(), label: "Strategy".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "start_time".to_string(), label: "Start Time".to_string(), required: true, value: None, visible: true, field_type: "datetime".to_string(), sensitive: false },
        UiField { name: "end_time".to_string(), label: "End Time".to_string(), required: true, value: None, visible: true, field_type: "datetime".to_string(), sensitive: false },
        UiField { name: "affected_monitors".to_string(), label: "Affected Monitors".to_string(), required: false, value: None, visible: true, field_type: "multi-select".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.maintenance_time", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&MaintenanceTimeComponent)->Vec<(String,String)>{
    logging::debug("ui.maintenance_time", "render_model_compat", "compat render");
