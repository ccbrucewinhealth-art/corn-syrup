// checklist translated from IncidentEditForm.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IncidentEditFormComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for IncidentEditFormComponent {
    fn default() -> Self {
        logging::debug("ui.incident_edit_form", "default", "init component with translated field schema");
        Self { source_path:"IncidentEditForm.vue", title:"事件編輯表單".to_string(), route:"/incident-edit-form".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl IncidentEditFormComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.incident_edit_form", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.incident_edit_form", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.incident_edit_form", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.incident_edit_form", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.incident_edit_form", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.incident_edit_form", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&IncidentEditFormComponent)->Vec<(String,String)>{
    logging::debug("ui.incident_edit_form", "render_model_compat", "compat render");
