// checklist translated from NotificationDialog.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NotificationDialogComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for NotificationDialogComponent {
    fn default() -> Self {
        logging::debug("ui.notification_dialog", "default", "init component with translated field schema");
        Self { source_path:"NotificationDialog.vue", title:"通知對話框".to_string(), route:"/notification-dialog".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl NotificationDialogComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.notification_dialog", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.notification_dialog", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.notification_dialog", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.notification_dialog", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.notification_dialog", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.notification_dialog", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&NotificationDialogComponent)->Vec<(String,String)>{
    logging::debug("ui.notification_dialog", "render_model_compat", "compat render");
