// checklist translated from APIKeyDialog.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ApiKeyDialogComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for ApiKeyDialogComponent {
    fn default() -> Self {
        logging::debug("ui.api_key_dialog", "default", "init component with translated field schema");
        Self { source_path:"APIKeyDialog.vue", title:"API Key 對話框".to_string(), route:"/api-key-dialog".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl ApiKeyDialogComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.api_key_dialog", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.api_key_dialog", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.api_key_dialog", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.api_key_dialog", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.api_key_dialog", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "name".to_string(), label: "Key Name".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "permissions".to_string(), label: "Permissions".to_string(), required: true, value: None, visible: true, field_type: "multi-select".to_string(), sensitive: false },
        UiField { name: "expires_at".to_string(), label: "Expires At".to_string(), required: false, value: None, visible: true, field_type: "datetime".to_string(), sensitive: false },
        UiField { name: "active".to_string(), label: "Active".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.api_key_dialog", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&ApiKeyDialogComponent)->Vec<(String,String)>{
    logging::debug("ui.api_key_dialog", "render_model_compat", "compat render");
