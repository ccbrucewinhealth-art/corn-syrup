// checklist translated from CertificateInfoRow.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CertificateInfoRowComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for CertificateInfoRowComponent {
    fn default() -> Self {
        logging::debug("ui.certificate_info_row", "default", "init component with translated field schema");
        Self { source_path:"CertificateInfoRow.vue", title:"憑證資訊列".to_string(), route:"/certificate-info-row".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl CertificateInfoRowComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.certificate_info_row", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.certificate_info_row", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.certificate_info_row", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.certificate_info_row", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.certificate_info_row", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "subject".to_string(), label: "Subject".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "issuer".to_string(), label: "Issuer".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "valid_from".to_string(), label: "Valid From".to_string(), required: true, value: None, visible: true, field_type: "datetime".to_string(), sensitive: false },
        UiField { name: "valid_to".to_string(), label: "Valid To".to_string(), required: true, value: None, visible: true, field_type: "datetime".to_string(), sensitive: false },
        UiField { name: "fingerprint".to_string(), label: "Fingerprint".to_string(), required: false, value: None, visible: true, field_type: "text".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.certificate_info_row", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&CertificateInfoRowComponent)->Vec<(String,String)>{
    logging::debug("ui.certificate_info_row", "render_model_compat", "compat render");
