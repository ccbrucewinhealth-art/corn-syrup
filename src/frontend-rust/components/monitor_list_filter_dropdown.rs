// checklist translated from MonitorListFilterDropdown.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MonitorListFilterDropdownComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for MonitorListFilterDropdownComponent {
    fn default() -> Self {
        logging::debug("ui.monitor_list_filter_dropdown", "default", "init component with translated field schema");
        Self { source_path:"MonitorListFilterDropdown.vue", title:"監控列表篩選下拉".to_string(), route:"/monitor-list-filter-dropdown".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl MonitorListFilterDropdownComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.monitor_list_filter_dropdown", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.monitor_list_filter_dropdown", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.monitor_list_filter_dropdown", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.monitor_list_filter_dropdown", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.monitor_list_filter_dropdown", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "monitor_id".to_string(), label: "Monitor ID".to_string(), required: false, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "name".to_string(), label: "Monitor Name".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "type".to_string(), label: "Monitor Type".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "interval".to_string(), label: "Interval Seconds".to_string(), required: true, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "conditions".to_string(), label: "Conditions".to_string(), required: false, value: None, visible: true, field_type: "expression".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.monitor_list_filter_dropdown", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&MonitorListFilterDropdownComponent)->Vec<(String,String)>{
    logging::debug("ui.monitor_list_filter_dropdown", "render_model_compat", "compat render");
