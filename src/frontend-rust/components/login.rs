// checklist translated from Login.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LoginComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for LoginComponent {
    fn default() -> Self {
        logging::debug("ui.login", "default", "init component with translated field schema");
        Self { source_path:"Login.vue", title:"登入".to_string(), route:"/login".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl LoginComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.login", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.login", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.login", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.login", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.login", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "username".to_string(), label: "Username".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "password".to_string(), label: "Password".to_string(), required: true, value: None, visible: true, field_type: "password".to_string(), sensitive: true },
        UiField { name: "remember_me".to_string(), label: "Remember Me".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false },
        UiField { name: "totp_token".to_string(), label: "2FA Token".to_string(), required: false, value: None, visible: true, field_type: "otp".to_string(), sensitive: true }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.login", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&LoginComponent)->Vec<(String,String)>{
    logging::debug("ui.login", "render_model_compat", "compat render");
