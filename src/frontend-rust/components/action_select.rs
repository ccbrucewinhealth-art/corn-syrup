// checklist translated from ActionSelect.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ActionSelectComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for ActionSelectComponent {
    fn default() -> Self {
        logging::debug("ui.action_select", "default", "init component with translated field schema");
        Self { source_path:"ActionSelect.vue", title:"行為選擇".to_string(), route:"/action-select".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl ActionSelectComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.action_select", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.action_select", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.action_select", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.action_select", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.action_select", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.action_select", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&ActionSelectComponent)->Vec<(String,String)>{
    logging::debug("ui.action_select", "render_model_compat", "compat render");
