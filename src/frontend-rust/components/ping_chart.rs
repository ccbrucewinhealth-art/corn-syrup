// checklist translated from PingChart.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PingChartComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for PingChartComponent {
    fn default() -> Self {
        logging::debug("ui.ping_chart", "default", "init component with translated field schema");
        Self { source_path:"PingChart.vue", title:"Ping 圖表".to_string(), route:"/ping-chart".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl PingChartComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.ping_chart", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.ping_chart", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.ping_chart", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.ping_chart", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.ping_chart", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.ping_chart", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&PingChartComponent)->Vec<(String,String)>{
    logging::debug("ui.ping_chart", "render_model_compat", "compat render");
