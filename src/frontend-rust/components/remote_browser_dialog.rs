// checklist translated from RemoteBrowserDialog.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RemoteBrowserDialogComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for RemoteBrowserDialogComponent {
    fn default() -> Self {
        logging::debug("ui.remote_browser_dialog", "default", "init component with translated field schema");
        Self { source_path:"RemoteBrowserDialog.vue", title:"遠端瀏覽器對話框".to_string(), route:"/remote-browser-dialog".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl RemoteBrowserDialogComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.remote_browser_dialog", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.remote_browser_dialog", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.remote_browser_dialog", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.remote_browser_dialog", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.remote_browser_dialog", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "name".to_string(), label: "Browser Name".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "url".to_string(), label: "Browser WS Endpoint".to_string(), required: true, value: None, visible: true, field_type: "url".to_string(), sensitive: true },
        UiField { name: "auth_token".to_string(), label: "Auth Token".to_string(), required: false, value: None, visible: true, field_type: "password".to_string(), sensitive: true }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.remote_browser_dialog", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&RemoteBrowserDialogComponent)->Vec<(String,String)>{
    logging::debug("ui.remote_browser_dialog", "render_model_compat", "compat render");
