// checklist translated from Uptime.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UptimeComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for UptimeComponent {
    fn default() -> Self {
        logging::debug("ui.uptime", "default", "init component with translated field schema");
        Self { source_path:"Uptime.vue", title:"正常運行時間".to_string(), route:"/uptime".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl UptimeComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.uptime", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.uptime", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.uptime", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.uptime", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.uptime", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.uptime", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&UptimeComponent)->Vec<(String,String)>{
    logging::debug("ui.uptime", "render_model_compat", "compat render");
