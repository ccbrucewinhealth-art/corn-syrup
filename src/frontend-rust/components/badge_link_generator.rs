// checklist translated from BadgeLinkGeneratorDialog.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BadgeLinkGeneratorComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for BadgeLinkGeneratorComponent {
    fn default() -> Self {
        logging::debug("ui.badge_link_generator", "default", "init component with translated field schema");
        Self { source_path:"BadgeLinkGeneratorDialog.vue", title:"徽章連結產生".to_string(), route:"/badge-link-generator".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl BadgeLinkGeneratorComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.badge_link_generator", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.badge_link_generator", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.badge_link_generator", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.badge_link_generator", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.badge_link_generator", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "title".to_string(), label: "Title".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "description".to_string(), label: "Description".to_string(), required: false, value: None, visible: true, field_type: "textarea".to_string(), sensitive: false },
        UiField { name: "enabled".to_string(), label: "Enabled".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false },
        UiField { name: "route".to_string(), label: "Route".to_string(), required: false, value: None, visible: true, field_type: "text".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.badge_link_generator", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&BadgeLinkGeneratorComponent)->Vec<(String,String)>{
    logging::debug("ui.badge_link_generator", "render_model_compat", "compat render");
