// checklist translated from Confirm.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConfirmComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for ConfirmComponent {
    fn default() -> Self {
        logging::debug("ui.confirm", "default", "init component with translated field schema");
        Self { source_path:"Confirm.vue", title:"確認對話框".to_string(), route:"/confirm".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl ConfirmComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.confirm", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.confirm", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.confirm", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.confirm", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.confirm", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "message".to_string(), label: "Message".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "confirm_text".to_string(), label: "Confirm Text".to_string(), required: false, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "danger".to_string(), label: "Danger".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.confirm", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&ConfirmComponent)->Vec<(String,String)>{
    logging::debug("ui.confirm", "render_model_compat", "compat render");
