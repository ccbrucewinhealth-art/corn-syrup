// checklist translated from MonitorListFilter.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MonitorListFilterComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for MonitorListFilterComponent {
    fn default() -> Self {
        logging::debug("ui.monitor_list_filter", "default", "init component with translated field schema");
        Self { source_path:"MonitorListFilter.vue", title:"監控列表篩選".to_string(), route:"/monitor-list-filter".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl MonitorListFilterComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.monitor_list_filter", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.monitor_list_filter", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.monitor_list_filter", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.monitor_list_filter", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.monitor_list_filter", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.monitor_list_filter", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&MonitorListFilterComponent)->Vec<(String,String)>{
    logging::debug("ui.monitor_list_filter", "render_model_compat", "compat render");
