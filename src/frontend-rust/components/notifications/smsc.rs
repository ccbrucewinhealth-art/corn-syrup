// checklist translated from SMSC.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmscComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmscComponent {
    fn default()->Self {
        logging::debug("notification-ui.smsc","default","init provider-specific notification form");
        Self{source_path:"SMSC.vue",provider:"smsc".to_string(),title:"SMSC".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmscComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.smsc","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.smsc","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.smsc","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.smsc","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.smsc","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmscComponent)->Vec<(String,String)>{ logging::debug("notification-ui.smsc","render_model_compat","compat render"); component.render_model() }
