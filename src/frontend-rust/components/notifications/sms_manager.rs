// checklist translated from SMSManager.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmsManagerComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmsManagerComponent {
    fn default()->Self {
        logging::debug("notification-ui.sms_manager","default","init provider-specific notification form");
        Self{source_path:"SMSManager.vue",provider:"sms_manager".to_string(),title:"SMS Manager".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmsManagerComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.sms_manager","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.sms_manager","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.sms_manager","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.sms_manager","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.sms_manager","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmsManagerComponent)->Vec<(String,String)>{ logging::debug("notification-ui.sms_manager","render_model_compat","compat render"); component.render_model() }
