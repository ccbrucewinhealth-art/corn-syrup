// checklist translated from Discord.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DiscordComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for DiscordComponent {
    fn default()->Self {
        logging::debug("notification-ui.discord","default","init provider-specific notification form");
        Self{source_path:"Discord.vue",provider:"discord".to_string(),title:"Discord".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl DiscordComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.discord","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.discord","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.discord","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.discord","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.discord","default_fields","build provider-specific fields");
    vec![
        ...
        NotificationField{name:"avatar_url".to_string(),label:"Avatar URL".to_string(),required:false,value:None,field_type:"url".to_string(),sensitive:false}
    ]
}

pub fn render_model(component:&DiscordComponent)->Vec<(String,String)>{ logging::debug("notification-ui.discord","render_model_compat","compat render"); component.render_model() }
