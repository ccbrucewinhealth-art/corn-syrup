// checklist translated from JiraServiceManagement.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct JiraSmComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for JiraSmComponent {
    fn default()->Self {
        logging::debug("notification-ui.jira_sm","default","init provider-specific notification form");
        Self{source_path:"JiraServiceManagement.vue",provider:"jira_sm".to_string(),title:"Jira SM".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl JiraSmComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.jira_sm","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.jira_sm","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.jira_sm","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.jira_sm","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.jira_sm","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&JiraSmComponent)->Vec<(String,String)>{ logging::debug("notification-ui.jira_sm","render_model_compat","compat render"); component.render_model() }
