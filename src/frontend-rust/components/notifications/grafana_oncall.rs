// checklist translated from GrafanaOncall.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GrafanaOncallComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for GrafanaOncallComponent {
    fn default()->Self {
        logging::debug("notification-ui.grafana_oncall","default","init provider-specific notification form");
        Self{source_path:"GrafanaOncall.vue",provider:"grafana_oncall".to_string(),title:"Grafana OnCall".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl GrafanaOncallComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.grafana_oncall","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.grafana_oncall","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.grafana_oncall","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.grafana_oncall","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.grafana_oncall","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&GrafanaOncallComponent)->Vec<(String,String)>{ logging::debug("notification-ui.grafana_oncall","render_model_compat","compat render"); component.render_model() }
