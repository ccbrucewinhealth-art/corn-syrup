// checklist translated from PagerDuty.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PagerDutyComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for PagerDutyComponent {
    fn default()->Self {
        logging::debug("notification-ui.pager_duty","default","init provider-specific notification form");
        Self{source_path:"PagerDuty.vue",provider:"pager_duty".to_string(),title:"PagerDuty".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl PagerDutyComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.pager_duty","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.pager_duty","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.pager_duty","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.pager_duty","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.pager_duty","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&PagerDutyComponent)->Vec<(String,String)>{ logging::debug("notification-ui.pager_duty","render_model_compat","compat render"); component.render_model() }
