// checklist translated from SevenIO.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SevenIoComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SevenIoComponent {
    fn default()->Self {
        logging::debug("notification-ui.seven_io","default","init provider-specific notification form");
        Self{source_path:"SevenIO.vue",provider:"seven_io".to_string(),title:"Seven.io".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SevenIoComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.seven_io","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.seven_io","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.seven_io","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.seven_io","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.seven_io","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SevenIoComponent)->Vec<(String,String)>{ logging::debug("notification-ui.seven_io","render_model_compat","compat render"); component.render_model() }
