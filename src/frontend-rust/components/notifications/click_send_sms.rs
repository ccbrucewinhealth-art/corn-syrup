// checklist translated from ClickSendSMS.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClickSendSmsComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for ClickSendSmsComponent {
    fn default()->Self {
        logging::debug("notification-ui.click_send_sms","default","init provider-specific notification form");
        Self{source_path:"ClickSendSMS.vue",provider:"click_send_sms".to_string(),title:"ClickSend SMS".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl ClickSendSmsComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.click_send_sms","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.click_send_sms","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.click_send_sms","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.click_send_sms","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.click_send_sms","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&ClickSendSmsComponent)->Vec<(String,String)>{ logging::debug("notification-ui.click_send_sms","render_model_compat","compat render"); component.render_model() }
