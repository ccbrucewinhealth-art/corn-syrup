// checklist translated from GoogleSheets.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GoogleSheetsComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for GoogleSheetsComponent {
    fn default()->Self {
        logging::debug("notification-ui.google_sheets","default","init provider-specific notification form");
        Self{source_path:"GoogleSheets.vue",provider:"google_sheets".to_string(),title:"Google Sheets".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl GoogleSheetsComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.google_sheets","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.google_sheets","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.google_sheets","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.google_sheets","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.google_sheets","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&GoogleSheetsComponent)->Vec<(String,String)>{ logging::debug("notification-ui.google_sheets","render_model_compat","compat render"); component.render_model() }
