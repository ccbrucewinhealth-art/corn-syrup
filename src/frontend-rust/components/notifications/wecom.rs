// checklist translated from WeCom.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct WecomComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for WecomComponent {
    fn default()->Self {
        logging::debug("notification-ui.wecom","default","init provider-specific notification form");
        Self{source_path:"WeCom.vue",provider:"wecom".to_string(),title:"企業微信".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl WecomComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.wecom","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.wecom","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.wecom","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.wecom","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.wecom","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&WecomComponent)->Vec<(String,String)>{ logging::debug("notification-ui.wecom","render_model_compat","compat render"); component.render_model() }
