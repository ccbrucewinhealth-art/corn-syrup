// checklist translated from GoAlert.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GoAlertComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for GoAlertComponent {
    fn default()->Self {
        logging::debug("notification-ui.go_alert","default","init provider-specific notification form");
        Self{source_path:"GoAlert.vue",provider:"go_alert".to_string(),title:"GoAlert".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl GoAlertComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.go_alert","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.go_alert","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.go_alert","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.go_alert","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.go_alert","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&GoAlertComponent)->Vec<(String,String)>{ logging::debug("notification-ui.go_alert","render_model_compat","compat render"); component.render_model() }
