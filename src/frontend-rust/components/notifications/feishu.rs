// checklist translated from Feishu.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FeishuComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for FeishuComponent {
    fn default()->Self {
        logging::debug("notification-ui.feishu","default","init provider-specific notification form");
        Self{source_path:"Feishu.vue",provider:"feishu".to_string(),title:"飛書".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl FeishuComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.feishu","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.feishu","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.feishu","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.feishu","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.feishu","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&FeishuComponent)->Vec<(String,String)>{ logging::debug("notification-ui.feishu","render_model_compat","compat render"); component.render_model() }
