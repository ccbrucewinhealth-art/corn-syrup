// checklist translated from Splunk.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SplunkComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SplunkComponent {
    fn default()->Self {
        logging::debug("notification-ui.splunk","default","init provider-specific notification form");
        Self{source_path:"Splunk.vue",provider:"splunk".to_string(),title:"Splunk".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SplunkComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.splunk","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.splunk","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.splunk","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.splunk","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.splunk","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SplunkComponent)->Vec<(String,String)>{ logging::debug("notification-ui.splunk","render_model_compat","compat render"); component.render_model() }
