// checklist translated from SMSPlanet.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmsPlanetComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmsPlanetComponent {
    fn default()->Self {
        logging::debug("notification-ui.sms_planet","default","init provider-specific notification form");
        Self{source_path:"SMSPlanet.vue",provider:"sms_planet".to_string(),title:"SMS Planet".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmsPlanetComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.sms_planet","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.sms_planet","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.sms_planet","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.sms_planet","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.sms_planet","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmsPlanetComponent)->Vec<(String,String)>{ logging::debug("notification-ui.sms_planet","render_model_compat","compat render"); component.render_model() }
