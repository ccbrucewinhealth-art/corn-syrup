// checklist translated from Squadcast.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SquadcastComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SquadcastComponent {
    fn default()->Self {
        logging::debug("notification-ui.squadcast","default","init provider-specific notification form");
        Self{source_path:"Squadcast.vue",provider:"squadcast".to_string(),title:"Squadcast".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SquadcastComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.squadcast","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.squadcast","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.squadcast","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.squadcast","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.squadcast","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SquadcastComponent)->Vec<(String,String)>{ logging::debug("notification-ui.squadcast","render_model_compat","compat render"); component.render_model() }
