// checklist translated from Teams.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TeamsComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for TeamsComponent {
    fn default()->Self {
        logging::debug("notification-ui.teams","default","init provider-specific notification form");
        Self{source_path:"Teams.vue",provider:"teams".to_string(),title:"Teams".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl TeamsComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.teams","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.teams","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.teams","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.teams","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.teams","default_fields","build provider-specific fields");
    vec![
        ...
        NotificationField{name:"summary".to_string(),label:"Summary".to_string(),required:false,value:None,field_type:"text".to_string(),sensitive:false}
    ]
}

pub fn render_model(component:&TeamsComponent)->Vec<(String,String)>{ logging::debug("notification-ui.teams","render_model_compat","compat render"); component.render_model() }
