// checklist translated from SMSPartner.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmsPartnerComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmsPartnerComponent {
    fn default()->Self {
        logging::debug("notification-ui.sms_partner","default","init provider-specific notification form");
        Self{source_path:"SMSPartner.vue",provider:"sms_partner".to_string(),title:"SMS Partner".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmsPartnerComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.sms_partner","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.sms_partner","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.sms_partner","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.sms_partner","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.sms_partner","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmsPartnerComponent)->Vec<(String,String)>{ logging::debug("notification-ui.sms_partner","render_model_compat","compat render"); component.render_model() }
