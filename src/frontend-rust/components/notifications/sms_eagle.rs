// checklist translated from SMSEagle.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmsEagleComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmsEagleComponent {
    fn default()->Self {
        logging::debug("notification-ui.sms_eagle","default","init provider-specific notification form");
        Self{source_path:"SMSEagle.vue",provider:"sms_eagle".to_string(),title:"SMSEagle".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmsEagleComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.sms_eagle","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.sms_eagle","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.sms_eagle","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.sms_eagle","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.sms_eagle","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmsEagleComponent)->Vec<(String,String)>{ logging::debug("notification-ui.sms_eagle","render_model_compat","compat render"); component.render_model() }
