// checklist translated from SMSIR.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmsIrComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SmsIrComponent {
    fn default()->Self {
        logging::debug("notification-ui.sms_ir","default","init provider-specific notification form");
        Self{source_path:"SMSIR.vue",provider:"sms_ir".to_string(),title:"SMS.ir".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SmsIrComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.sms_ir","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.sms_ir","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.sms_ir","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.sms_ir","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.sms_ir","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&SmsIrComponent)->Vec<(String,String)>{ logging::debug("notification-ui.sms_ir","render_model_compat","compat render"); component.render_model() }
