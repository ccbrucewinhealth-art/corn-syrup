// checklist translated from Slack.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SlackComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for SlackComponent {
    fn default()->Self {
        logging::debug("notification-ui.slack","default","init provider-specific notification form");
        Self{source_path:"Slack.vue",provider:"slack".to_string(),title:"Slack".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl SlackComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.slack","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.slack","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.slack","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.slack","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.slack","default_fields","build provider-specific fields");
    vec![
        ...
        NotificationField{name:"channel".to_string(),label:"Channel".to_string(),required:false,value:None,field_type:"text".to_string(),sensitive:false},
...
pub fn render_model(component:&SlackComponent)->Vec<(String,String)>{ logging::debug("notification-ui.slack","render_model_compat","compat render"); component.render_model() }
