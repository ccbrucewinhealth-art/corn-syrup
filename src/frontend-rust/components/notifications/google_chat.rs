// checklist translated from GoogleChat.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GoogleChatComponent { pub source_path:&'static str, pub provider:String, pub title:String, pub fields:Vec<NotificationField>, pub errors:Vec<String> }

impl Default for GoogleChatComponent {
    fn default()->Self {
        logging::debug("notification-ui.google_chat","default","init provider-specific notification form");
        Self{source_path:"GoogleChat.vue",provider:"google_chat".to_string(),title:"Google Chat".to_string(),fields:default_fields(),errors:Vec::new()}
    }
}

impl GoogleChatComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("notification-ui.google_chat","validate",format!("fields={}",self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("notification-ui.google_chat","set_field",name);
    ...
    pub fn build_payload(&self)->Result<NotificationPayload,String>{
        logging::debug("notification-ui.google_chat","build_payload","compose provider config payload"); self.validate()?;
    ...
    pub fn render_model(&self)->Vec<(String,String)>{
        logging::debug("notification-ui.google_chat","render_model","render notification form with masked secrets");
...
pub fn default_fields()->Vec<NotificationField>{
    logging::debug("notification-ui.google_chat","default_fields","build provider-specific fields");
    vec![
...
pub fn render_model(component:&GoogleChatComponent)->Vec<(String,String)>{ logging::debug("notification-ui.google_chat","render_model_compat","compat render"); component.render_model() }
