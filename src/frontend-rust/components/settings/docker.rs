// checklist translated from Docker.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DockerComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for DockerComponent {
    fn default() -> Self {
        logging::debug("ui.docker", "default", "init component with translated field schema");
        Self { source_path:"Docker.vue", title:"Docker 設定".to_string(), route:"/settings/docker".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl DockerComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.docker", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.docker", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.docker", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.docker", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.docker", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.docker", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&DockerComponent)->Vec<(String,String)>{
    logging::debug("ui.docker", "render_model_compat", "compat render");
