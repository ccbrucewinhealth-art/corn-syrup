// checklist translated from Tags.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TagsComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for TagsComponent {
    fn default() -> Self {
        logging::debug("ui.tags", "default", "init component with translated field schema");
        Self { source_path:"Tags.vue", title:"標籤設定".to_string(), route:"/settings/tags".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl TagsComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.tags", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.tags", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.tags", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.tags", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.tags", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.tags", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&TagsComponent)->Vec<(String,String)>{
    logging::debug("ui.tags", "render_model_compat", "compat render");
