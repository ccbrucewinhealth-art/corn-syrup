// checklist translated from ReverseProxy.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ReverseProxyComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for ReverseProxyComponent {
    fn default() -> Self {
        logging::debug("ui.reverse_proxy", "default", "init component with translated field schema");
        Self { source_path:"ReverseProxy.vue", title:"反向代理".to_string(), route:"/settings/reverse-proxy".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl ReverseProxyComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.reverse_proxy", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.reverse_proxy", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.reverse_proxy", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.reverse_proxy", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.reverse_proxy", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "protocol".to_string(), label: "Protocol".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "host".to_string(), label: "Host".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "port".to_string(), label: "Port".to_string(), required: true, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "username".to_string(), label: "Username".to_string(), required: false, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "password".to_string(), label: "Password".to_string(), required: false, value: None, visible: true, field_type: "password".to_string(), sensitive: true }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.reverse_proxy", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&ReverseProxyComponent)->Vec<(String,String)>{
    logging::debug("ui.reverse_proxy", "render_model_compat", "compat render");
