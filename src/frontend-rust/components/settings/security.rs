// checklist translated from Security.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SecurityComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for SecurityComponent {
    fn default() -> Self {
        logging::debug("ui.security", "default", "init component with translated field schema");
        Self { source_path:"Security.vue", title:"安全性設定".to_string(), route:"/settings/security".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl SecurityComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.security", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.security", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.security", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.security", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.security", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "password_policy".to_string(), label: "Password Policy".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "two_fa_required".to_string(), label: "Require 2FA".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false },
        UiField { name: "session_timeout".to_string(), label: "Session Timeout".to_string(), required: true, value: None, visible: true, field_type: "number".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.security", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&SecurityComponent)->Vec<(String,String)>{
    logging::debug("ui.security", "render_model_compat", "compat render");
