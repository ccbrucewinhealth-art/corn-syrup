// checklist translated from lang.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LangComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for LangComponent {
    fn default() -> Self {
        logging::debug("ui.lang", "default", "init component with translated field schema");
        Self { source_path:"lang.js", title:"語言 Mixin".to_string(), route:"/mixin/lang".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl LangComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.lang", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.lang", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.lang", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.lang", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.lang", "default_fields", "build component-specific schema");
    vec![
        ...
        UiField { name: "fallback_locale".to_string(), label: "Fallback Locale".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.lang", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&LangComponent)->Vec<(String,String)>{
    logging::debug("ui.lang", "render_model_compat", "compat render");
