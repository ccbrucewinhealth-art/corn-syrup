// checklist translated from theme.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ThemeComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for ThemeComponent {
    fn default() -> Self {
        logging::debug("ui.theme", "default", "init component with translated field schema");
        Self { source_path:"theme.js", title:"主題 Mixin".to_string(), route:"/mixin/theme".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl ThemeComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.theme", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.theme", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.theme", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.theme", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.theme", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "theme".to_string(), label: "Theme".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "dark_mode".to_string(), label: "Dark Mode".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.theme", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&ThemeComponent)->Vec<(String,String)>{
    logging::debug("ui.theme", "render_model_compat", "compat render");
