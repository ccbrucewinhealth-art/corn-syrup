// checklist translated from mobile.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MobileComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for MobileComponent {
    fn default() -> Self {
        logging::debug("ui.mobile", "default", "init component with translated field schema");
        Self { source_path:"mobile.js", title:"行動裝置 Mixin".to_string(), route:"/mixin/mobile".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl MobileComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.mobile", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.mobile", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.mobile", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.mobile", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.mobile", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "viewport_width".to_string(), label: "Viewport Width".to_string(), required: true, value: None, visible: true, field_type: "number".to_string(), sensitive: false },
        UiField { name: "is_mobile".to_string(), label: "Is Mobile".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.mobile", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&MobileComponent)->Vec<(String,String)>{
    logging::debug("ui.mobile", "render_model_compat", "compat render");
