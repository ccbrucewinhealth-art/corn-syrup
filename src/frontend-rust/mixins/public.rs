// checklist translated from public.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PublicComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for PublicComponent {
    fn default() -> Self {
        logging::debug("ui.public", "default", "init component with translated field schema");
        Self { source_path:"public.js", title:"公開 Mixin".to_string(), route:"/mixin/public".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl PublicComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.public", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.public", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.public", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.public", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.public", "default_fields", "build component-specific schema");
    vec![
        ...
        UiField { name: "state".to_string(), label: "State".to_string(), required: false, value: None, visible: true, field_type: "json".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.public", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&PublicComponent)->Vec<(String,String)>{
    logging::debug("ui.public", "render_model_compat", "compat render");
