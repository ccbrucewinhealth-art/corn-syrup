// checklist translated from socket.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SocketComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for SocketComponent {
    fn default() -> Self {
        logging::debug("ui.socket", "default", "init component with translated field schema");
        Self { source_path:"socket.js", title:"Socket Mixin".to_string(), route:"/mixin/socket".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl SocketComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.socket", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.socket", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.socket", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.socket", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.socket", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "socket_url".to_string(), label: "Socket URL".to_string(), required: true, value: None, visible: true, field_type: "url".to_string(), sensitive: false },
        UiField { name: "connected".to_string(), label: "Connected".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false },
        UiField { name: "last_event".to_string(), label: "Last Event".to_string(), required: false, value: None, visible: true, field_type: "text".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.socket", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&SocketComponent)->Vec<(String,String)>{
    logging::debug("ui.socket", "render_model_compat", "compat render");
