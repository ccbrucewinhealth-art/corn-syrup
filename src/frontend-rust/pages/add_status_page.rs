// checklist No.272 translated from backend/pages/AddStatusPage.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AddStatusPageComponent {
...
impl Default for AddStatusPageComponent {
    fn default() -> Self {
        logging::debug("frontend.add_status_page", "default", "create default component state");
        Self { source_path: "backend/pages/AddStatusPage.vue", title: "新增狀態頁面".to_string(), route: "/add_status_page".to_string(), loading: false, error: None, fields: default_fields(), actions: default_actions() }
    }
}

impl AddStatusPageComponent {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("frontend.add_status_page", "validate", format!("route={}", self.route));
    ...
    pub fn apply_route(&mut self, route: impl Into<String>) -> Result<(), String> {
        ...
        logging::debug("frontend.add_status_page", "apply_route", &route);
    ...
    pub fn set_loading(&mut self, loading: bool) {
        logging::debug("frontend.add_status_page", "set_loading", loading.to_string());
        self.loading = loading;
    }

    pub fn set_error(&mut self, error: Option<String>) {
        logging::debug("frontend.add_status_page", "set_error", error.clone().unwrap_or_else(|| "none".to_string()));
        self.error = error;
    }

    pub fn update_field(&mut self, name: &str, value: impl Into<String>) -> Result<(), String> {
        logging::debug("frontend.add_status_page", "update_field", name);
    ...
    pub fn render_model(&self) -> Result<Vec<(String, String)>, String> {
        ...
        logging::debug("frontend.add_status_page", "render_model", format!("fields={}, actions={}", self.fields.len(), self.actions.len()));
...
pub fn default_fields() -> Vec<UiField> {
    logging::debug("frontend.add_status_page", "default_fields", "build fields from translated component contract");
...
pub fn default_actions() -> Vec<UiAction> {
    logging::debug("frontend.add_status_page", "default_actions", "build action list");
...
pub fn render_model(component: &AddStatusPageComponent) -> Vec<(String, String)> {
    logging::debug("frontend.add_status_page", "render_model_compat", "compat wrapper");
