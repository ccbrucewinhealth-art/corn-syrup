// checklist translated from NotFound.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NotFoundComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for NotFoundComponent {
    fn default() -> Self {
        logging::debug("ui.not_found", "default", "init component with translated field schema");
        Self { source_path:"NotFound.vue", title:"404頁面".to_string(), route:"/not-found".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl NotFoundComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.not_found", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.not_found", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.not_found", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.not_found", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.not_found", "default_fields", "build component-specific schema");
    vec![
...
pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.not_found", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&NotFoundComponent)->Vec<(String,String)>{
    logging::debug("ui.not_found", "render_model_compat", "compat render");
