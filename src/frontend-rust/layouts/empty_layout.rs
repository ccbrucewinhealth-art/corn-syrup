// checklist translated from EmptyLayout.vue
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EmptyLayoutComponent { pub source_path:&'static str, pub title:String, pub route:String, pub loading:bool, pub dirty:bool, pub error:Option<String>, pub fields:Vec<UiField>, pub actions:Vec<UiAction>, pub emitted:Vec<String> }

impl Default for EmptyLayoutComponent {
    fn default() -> Self {
        logging::debug("ui.empty_layout", "default", "init component with translated field schema");
        Self { source_path:"EmptyLayout.vue", title:"空白布局".to_string(), route:"/layout/empty-layout".to_string(), loading:false, dirty:false, error:None, fields:default_fields(), actions:default_actions(), emitted:Vec::new() }
    }
}

impl EmptyLayoutComponent {
    pub fn validate(&self)->Result<(),String>{
        logging::debug("ui.empty_layout", "validate", format!("route={}, fields={}", self.route, self.fields.len()));
    ...
    pub fn set_field(&mut self,name:&str,value:impl Into<String>)->Result<(),String>{
        logging::debug("ui.empty_layout", "set_field", name);
    ...
    pub fn dispatch_action(&mut self,event:UiEvent)->Result<(),String>{
        logging::debug("ui.empty_layout", "dispatch_action", format!("{:?}",event));
        match event {
    ...
    pub fn render_model(&self)->Result<Vec<(String,String)>,String>{
        self.validate()?; logging::debug("ui.empty_layout", "render_model", format!("actions={}", self.actions.len()));
...
pub fn default_fields()->Vec<UiField>{
    logging::debug("ui.empty_layout", "default_fields", "build component-specific schema");
    vec![
        UiField { name: "active_route".to_string(), label: "Active Route".to_string(), required: true, value: None, visible: true, field_type: "text".to_string(), sensitive: false },
        UiField { name: "sidebar_collapsed".to_string(), label: "Sidebar Collapsed".to_string(), required: false, value: None, visible: true, field_type: "checkbox".to_string(), sensitive: false },
        UiField { name: "theme".to_string(), label: "Theme".to_string(), required: false, value: None, visible: true, field_type: "select".to_string(), sensitive: false },
        UiField { name: "locale".to_string(), label: "Locale".to_string(), required: true, value: None, visible: true, field_type: "select".to_string(), sensitive: false }
    ]
}

pub fn default_actions()->Vec<UiAction>{
    logging::debug("ui.empty_layout", "default_actions", "build supported actions");
    vec![
...
pub fn render_model(component:&EmptyLayoutComponent)->Vec<(String,String)>{
    logging::debug("ui.empty_layout", "render_model_compat", "compat render");
