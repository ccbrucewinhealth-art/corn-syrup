// checklist No.212 translated from server/notification-providers/stackfield.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StackfieldNotification {
...
impl StackfieldNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/stackfield.js";
    pub const PROVIDER: &'static str = "stackfield";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.stackfield", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.stackfield", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.stackfield", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.stackfield", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.stackfield", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.stackfield", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn stackfield_run(provider: &StackfieldNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.stackfield", "run", "validate and build provider payload");
