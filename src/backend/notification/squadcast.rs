// checklist No.211 translated from server/notification-providers/squadcast.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SquadcastNotification {
...
impl SquadcastNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/squadcast.js";
    pub const PROVIDER: &'static str = "squadcast";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.squadcast", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.squadcast", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.squadcast", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.squadcast", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.squadcast", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.squadcast", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn squadcast_run(provider: &SquadcastNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.squadcast", "run", "validate and build provider payload");
