// checklist No.215 translated from server/notification-providers/teltonika.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TeltonikaNotification {
...
impl TeltonikaNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/teltonika.js";
    pub const PROVIDER: &'static str = "teltonika";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.teltonika", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.teltonika", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.teltonika", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.teltonika", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.teltonika", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.teltonika", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn teltonika_run(provider: &TeltonikaNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.teltonika", "run", "validate and build provider payload");
