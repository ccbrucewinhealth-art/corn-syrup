// checklist No.221 translated from server/notification-providers/zoho-cliq.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ZohoCliqNotification {
...
impl ZohoCliqNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/zoho-cliq.js";
    pub const PROVIDER: &'static str = "zoho_cliq";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.zoho_cliq", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.zoho_cliq", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.zoho_cliq", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.zoho_cliq", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.zoho_cliq", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.zoho_cliq", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn zoho_cliq_run(provider: &ZohoCliqNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.zoho_cliq", "run", "validate and build provider payload");
