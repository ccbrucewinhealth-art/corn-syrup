// checklist No.207 translated from server/notification-providers/sevenio.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SevenioNotification {
...
impl SevenioNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/sevenio.js";
    pub const PROVIDER: &'static str = "sevenio";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.sevenio", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.sevenio", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.sevenio", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.sevenio", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.sevenio", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.sevenio", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn sevenio_run(provider: &SevenioNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.sevenio", "run", "validate and build provider payload");
