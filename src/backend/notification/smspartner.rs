// checklist No.208 translated from server/notification-providers/smspartner.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SmspartnerNotification {
...
impl SmspartnerNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/smspartner.js";
    pub const PROVIDER: &'static str = "smspartner";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.smspartner", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.smspartner", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.smspartner", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.smspartner", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.smspartner", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.smspartner", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn smspartner_run(provider: &SmspartnerNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.smspartner", "run", "validate and build provider payload");
