// checklist No.220 translated from server/notification-providers/wpush.js
...
impl WpushNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/wpush.js";
    pub const PROVIDER: &'static str = "wpush";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.wpush", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.wpush", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.wpush", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.wpush", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.wpush", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.wpush", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn wpush_run(provider: &WpushNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.wpush", "run", "validate and build provider payload");
