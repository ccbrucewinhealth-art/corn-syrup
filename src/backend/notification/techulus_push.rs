// checklist No.213 translated from server/notification-providers/techulus-push.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TechulusPushNotification {
...
impl TechulusPushNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/techulus-push.js";
    pub const PROVIDER: &'static str = "techulus_push";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.techulus_push", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.techulus_push", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.techulus_push", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.techulus_push", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.techulus_push", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.techulus_push", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn techulus_push_run(provider: &TechulusPushNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.techulus_push", "run", "validate and build provider payload");
