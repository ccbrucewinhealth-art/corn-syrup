// checklist No.206 translated from server/notification-providers/serwersms.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SerwersmsNotification {
...
impl SerwersmsNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/serwersms.js";
    pub const PROVIDER: &'static str = "serwersms";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.serwersms", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.serwersms", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.serwersms", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.serwersms", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.serwersms", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.serwersms", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn serwersms_run(provider: &SerwersmsNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.serwersms", "run", "validate and build provider payload");
