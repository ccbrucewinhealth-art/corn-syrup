// checklist No.224 translated from server/notification-providers/HaloPSA.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HalopsaNotification {
...
impl HalopsaNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/HaloPSA.js";
    pub const PROVIDER: &'static str = "halopsa";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.halopsa", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.halopsa", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.halopsa", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.halopsa", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.halopsa", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.halopsa", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn halopsa_run(provider: &HalopsaNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.halopsa", "run", "validate and build provider payload");
