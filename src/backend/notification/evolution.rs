// checklist No.225 translated from server/notification-providers/evolution.js
...
impl EvolutionNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/evolution.js";
    pub const PROVIDER: &'static str = "evolution";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.evolution", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.evolution", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.evolution", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.evolution", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.evolution", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.evolution", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn evolution_run(provider: &EvolutionNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.evolution", "run", "validate and build provider payload");
