// checklist No.210 translated from server/notification-providers/spugpush.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SpugpushNotification {
...
impl SpugpushNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/spugpush.js";
    pub const PROVIDER: &'static str = "spugpush";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.spugpush", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.spugpush", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.spugpush", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.spugpush", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.spugpush", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.spugpush", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn spugpush_run(provider: &SpugpushNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.spugpush", "run", "validate and build provider payload");
