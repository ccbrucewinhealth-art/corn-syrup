// checklist No.232 translated from server/notification-providers/nextcloudtalk.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NextcloudtalkNotification {
...
impl NextcloudtalkNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/nextcloudtalk.js";
    pub const PROVIDER: &'static str = "nextcloudtalk";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.nextcloudtalk", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.nextcloudtalk", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.nextcloudtalk", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.nextcloudtalk", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.nextcloudtalk", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.nextcloudtalk", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn nextcloudtalk_run(provider: &NextcloudtalkNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.nextcloudtalk", "run", "validate and build provider payload");
