// checklist No.233 translated from server/notification-providers/Webpush.js
...
impl WebpushNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/Webpush.js";
    pub const PROVIDER: &'static str = "webpush";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.webpush", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.webpush", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.webpush", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.webpush", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.webpush", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.webpush", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn webpush_run(provider: &WebpushNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.webpush", "run", "validate and build provider payload");
