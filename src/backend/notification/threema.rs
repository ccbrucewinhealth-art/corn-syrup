// checklist No.216 translated from server/notification-providers/threema.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ThreemaNotification {
...
impl ThreemaNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/threema.js";
    pub const PROVIDER: &'static str = "threema";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.threema", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.threema", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.threema", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.threema", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.threema", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.threema", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn threema_run(provider: &ThreemaNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.threema", "run", "validate and build provider payload");
