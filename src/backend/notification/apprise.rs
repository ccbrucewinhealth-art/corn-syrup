// checklist No.222 translated from server/notification-providers/apprise.js
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AppriseNotification {
...
impl AppriseNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/apprise.js";
    pub const PROVIDER: &'static str = "apprise";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.apprise", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.apprise", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.apprise", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.apprise", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.apprise", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.apprise", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn apprise_run(provider: &AppriseNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.apprise", "run", "validate and build provider payload");
