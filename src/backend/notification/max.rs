// checklist No.231 translated from server/notification-providers/max.js
...
impl MaxNotification {
    pub const SOURCE_PATH: &'static str = "server/notification-providers/max.js";
    pub const PROVIDER: &'static str = "max";

    pub fn new(endpoint: Option<String>, token: Option<String>) -> Self {
        logging::debug("notification.max", "new", "create provider configuration");
    ...
    pub fn validate(&self, message: &NotificationMessage) -> Result<(), String> {
        logging::debug("notification.max", "validate", format!("title_len={}, body_len={}", message.title.len(), message.body.len()));
    ...
    pub fn normalized_endpoint(&self) -> String {
        logging::debug("notification.max", "normalized_endpoint", "resolve endpoint fallback");
    ...
    pub fn build_fields(&self, message: &NotificationMessage) -> Vec<(String, String)> {
        ...
        logging::debug("notification.max", "build_fields", "compose title/body/status/monitor fields");
    ...
    pub fn build_payload(&self, message: &NotificationMessage) -> Result<NotificationPayload, String> {
        ...
        logging::debug("notification.max", "build_payload", "build HTTP request payload");
    ...
    pub fn send_plan(&self, message: &NotificationMessage) -> Result<Vec<(String, String)>, String> {
        logging::debug("notification.max", "send_plan", "convert payload to adapter-neutral plan");
...
pub fn max_run(provider: &MaxNotification, message: &NotificationMessage) -> Result<NotificationPayload, String> {
    logging::debug("notification.max", "run", "validate and build provider payload");
