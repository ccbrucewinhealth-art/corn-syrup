...
// checklist No.45 translated from server/monitor-types/rabbitmq.js
// 轉譯分析：RabbitMQ 監控，依來源檢查 Management API/AMQP 目標與回應狀態。
...
#[derive(Debug, Clone)]
pub struct RabbitmqResult {
    pub state: MonitorState,
    pub message: String,
    pub ping_ms: Option<u128>,
}

impl Default for RabbitmqContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/rabbitmq.js",
...
impl RabbitmqContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.rabbitmq", "validate", "enter");
...
pub fn rabbitmq_run(ctx: &RabbitmqContext) -> Result<RabbitmqResult, String> {
    logging::debug("auto.rabbitmq", "rabbitmq_run", "enter");
    ctx.validate()?;
