...
// checklist No.44 translated from server/monitor-types/postgres.js
// 轉譯分析：PostgreSQL 監控，依來源建立連線設定、執行查詢並判定成功。
...
impl Default for PostgresContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/postgres.js",
...
impl PostgresContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.postgres", "validate", "enter");
...
pub fn postgres_run(ctx: &PostgresContext) -> Result<PostgresResult, String> {
    logging::debug("auto.postgres", "postgres_run", "enter");
    ctx.validate()?;
