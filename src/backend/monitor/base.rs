...
// checklist No.40 translated from server/monitor-types/monitor-type.js
// 轉譯分析：監控類型基底，依來源提供名稱、條件支援與自訂狀態能力描述。
...
impl Default for BaseContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/monitor-type.js",
            name: "base".to_string(),
            target: String::new(),
            expected: None,
            observed: None,
...
impl BaseContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.base", "validate", "enter");
...
pub fn base_run(ctx: &BaseContext) -> Result<BaseResult, String> {
    logging::debug("auto.base", "base_run", "enter");
    ctx.validate()?;
