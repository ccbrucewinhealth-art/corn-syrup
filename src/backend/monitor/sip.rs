...
// checklist No.52 translated from server/monitor-types/sip-options.js
// 轉譯分析：SIP OPTIONS 監控，依來源傳送 OPTIONS 並檢查回應碼。
...
impl Default for SipContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/sip-options.js",
            name: "sip".to_string(),
            target: String::new(),
            expected: None,
            observed: None,
...
impl SipContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.sip", "validate", "enter");
...
pub fn sip_run(ctx: &SipContext) -> Result<SipResult, String> {
    logging::debug("auto.sip", "sip_run", "enter");
    ctx.validate()?;
