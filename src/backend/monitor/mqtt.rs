...
// checklist No.41 translated from server/monitor-types/mqtt.js
// 轉譯分析：MQTT 監控，依來源連線 broker、訂閱 topic 並比對 payload。
...
impl Default for MqttContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/mqtt.js",
            name: "mqtt".to_string(),
            target: String::new(),
            expected: None,
            observed: None,
...
impl MqttContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.mqtt", "validate", "enter");
...
pub fn mqtt_run(ctx: &MqttContext) -> Result<MqttResult, String> {
    logging::debug("auto.mqtt", "mqtt_run", "enter");
    ctx.validate()?;
