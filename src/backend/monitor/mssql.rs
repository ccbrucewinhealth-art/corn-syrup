...
// checklist No.42 translated from server/monitor-types/mssql.js
// 轉譯分析：MSSQL 監控，依來源建立連線設定、執行查詢並判定成功。
...
#[derive(Debug, Clone)]
pub struct MssqlResult {
    pub state: MonitorState,
    pub message: String,
    pub ping_ms: Option<u128>,
}

impl Default for MssqlContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/mssql.js",
...
impl MssqlContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.mssql", "validate", "enter");
...
pub fn mssql_run(ctx: &MssqlContext) -> Result<MssqlResult, String> {
    logging::debug("auto.mssql", "mssql_run", "enter");
    ctx.validate()?;
