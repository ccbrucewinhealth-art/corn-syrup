...
// checklist No.48 translated from server/monitor-types/snmp.js
// 轉譯分析：SNMP 監控，依來源以 OID 查詢結果與條件運算子判定。
...
impl Default for SnmpContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/snmp.js",
            name: "snmp".to_string(),
            target: String::new(),
            expected: None,
            observed: None,
...
impl SnmpContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.snmp", "validate", "enter");
...
pub fn snmp_run(ctx: &SnmpContext) -> Result<SnmpResult, String> {
    logging::debug("auto.snmp", "snmp_run", "enter");
    ctx.validate()?;
