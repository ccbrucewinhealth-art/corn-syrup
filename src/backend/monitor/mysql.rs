...
// checklist No.43 translated from server/monitor-types/mysql.js
// 轉譯分析：MySQL 監控，依來源建立連線設定、執行查詢並判定成功。
...
impl Default for MysqlContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/mysql.js",
...
impl MysqlContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.mysql", "validate", "enter");
...
pub fn mysql_run(ctx: &MysqlContext) -> Result<MysqlResult, String> {
    logging::debug("auto.mysql", "mysql_run", "enter");
    ctx.validate()?;
