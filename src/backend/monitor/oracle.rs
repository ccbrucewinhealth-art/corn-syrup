...
// checklist No.36 translated from server/monitor-types/oracledb.js
// 轉譯分析：OracleDB 監控，依來源組裝連線描述、執行查詢並以成功列數判定。
...
impl Default for OracleContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/oracledb.js",
...
impl OracleContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.oracle", "validate", "enter");
...
pub fn oracle_run(ctx: &OracleContext) -> Result<OracleResult, String> {
    logging::debug("auto.oracle", "oracle_run", "enter");
    ctx.validate()?;
