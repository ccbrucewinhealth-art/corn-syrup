...
// checklist No.53 translated from server/monitor-types/mongodb.js
// 轉譯分析：MongoDB 監控，依來源建立連線並執行 ping 命令。
...
#[derive(Debug, Clone)]
pub struct MongodbResult {
    pub state: MonitorState,
    pub message: String,
    pub ping_ms: Option<u128>,
}

impl Default for MongodbContext {
    fn default() -> Self {
        Self {
            source_path: "server/monitor-types/mongodb.js",
...
impl MongodbContext {
    pub fn validate(&self) -> Result<(), String> {
        logging::debug("auto.mongodb", "validate", "enter");
...
pub fn mongodb_run(ctx: &MongodbContext) -> Result<MongodbResult, String> {
    logging::debug("auto.mongodb", "mongodb_run", "enter");
    ctx.validate()?;
