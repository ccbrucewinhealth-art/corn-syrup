// checklist No.63 translated from server/model/group.js
// 轉譯說明：群組模型。
// 來源摘要：lines=49；imports=redbean-node/dist/bean-model, redbean-node；classes=Group；functions=require, toPublicJSON, getMonitorList, for, push, convertToBeans, getAll。
...
pub enum GroupState { Active, Inactive, Deleted }
...
impl GroupModel {
    pub const SOURCE_PATH: &'static str = "server/model/group.js";

    pub fn new(id: i64, name: impl Into<String>) -> Self {
        logging::debug("model.group", "new", format!("id={id}"));
        Self { id, name: name.into(), state: GroupState::Active, created_date: None, updated_date: None, fields: Vec::new() }
    }

    pub fn validate(&self) -> Result<(), String> {
        // 參考系統 BeanModel 寫入前會檢查必要欄位；Rust 版集中在 validate，避免空名稱資料流入 socket/router。
        logging::debug("model.group", "validate", &self.name);
        if self.name.trim().is_empty() { return Err("group name is required".to_string()); }
        Ok(())
    }

    pub fn set_field(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        let key = key.into();
        logging::debug("model.group", "set_field", &key);
        self.fields.push((key, value.into()));
        self
    }

    pub fn to_json_pairs(&self) -> Vec<(String, String)> {
        logging::debug("model.group", "to_json_pairs", format!("field_count={}", self.fields.len()));
...
pub fn group_run(model: &GroupModel) -> Result<Vec<(String, String)>, String> {
    logging::debug("model.group", "run", "validate and serialize");
    model.validate()?;
