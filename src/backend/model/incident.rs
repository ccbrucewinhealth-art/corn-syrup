// checklist No.72 translated from server/model/incident.js
// 轉譯說明：事件模型。
// 來源摘要：lines=36；imports=redbean-node/dist/bean-model, redbean-node, dayjs；classes=Incident；functions=require, resolve, isoDateTime, utc, store, toPublicJSON。
...
pub enum IncidentState { Active, Inactive, Deleted }
...
impl IncidentModel {
    pub const SOURCE_PATH: &'static str = "server/model/incident.js";

    pub fn new(id: i64, name: impl Into<String>) -> Self {
        logging::debug("model.incident", "new", format!("id={id}"));
        Self { id, name: name.into(), state: IncidentState::Active, created_date: None, updated_date: None, fields: Vec::new() }
    }

    pub fn validate(&self) -> Result<(), String> {
        // 參考系統 BeanModel 寫入前會檢查必要欄位；Rust 版集中在 validate，避免空名稱資料流入 socket/router。
        logging::debug("model.incident", "validate", &self.name);
        if self.name.trim().is_empty() { return Err("incident name is required".to_string()); }
        Ok(())
    }

    pub fn set_field(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        let key = key.into();
        logging::debug("model.incident", "set_field", &key);
        self.fields.push((key, value.into()));
        self
    }

    pub fn to_json_pairs(&self) -> Vec<(String, String)> {
        logging::debug("model.incident", "to_json_pairs", format!("field_count={}", self.fields.len()));
...
pub fn incident_run(model: &IncidentModel) -> Result<Vec<(String, String)>, String> {
    logging::debug("model.incident", "run", "validate and serialize");
    model.validate()?;
