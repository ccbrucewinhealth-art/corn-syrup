// checklist No.62 translated from server/model/user.js
// 轉譯說明：使用者模型。
// 來源摘要：lines=52；imports=redbean-node/dist/bean-model, ../password-hash, redbean-node, jsonwebtoken, ../util-server；classes=User；functions=require, resetPassword, exec, generate, createJWT, sign, shake256。
...
pub enum UserState { Active, Inactive, Deleted }
...
impl UserModel {
    pub const SOURCE_PATH: &'static str = "server/model/user.js";

    pub fn new(id: i64, name: impl Into<String>) -> Self {
        logging::debug("model.user", "new", format!("id={id}"));
        Self { id, name: name.into(), state: UserState::Active, created_date: None, updated_date: None, fields: Vec::new() }
    }

    pub fn validate(&self) -> Result<(), String> {
        // 參考系統 BeanModel 寫入前會檢查必要欄位；Rust 版集中在 validate，避免空名稱資料流入 socket/router。
        logging::debug("model.user", "validate", &self.name);
        if self.name.trim().is_empty() { return Err("user name is required".to_string()); }
        Ok(())
    }

    pub fn set_field(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        let key = key.into();
        logging::debug("model.user", "set_field", &key);
        self.fields.push((key, value.into()));
        self
    }

    pub fn to_json_pairs(&self) -> Vec<(String, String)> {
        logging::debug("model.user", "to_json_pairs", format!("field_count={}", self.fields.len()));
...
pub fn user_run(model: &UserModel) -> Result<Vec<(String, String)>, String> {
    logging::debug("model.user", "run", "validate and serialize");
    model.validate()?;
