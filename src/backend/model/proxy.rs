// checklist No.69 translated from server/model/proxy.js
// 轉譯說明：代理模型。
// 來源摘要：lines=25；imports=redbean-node/dist/bean-model；classes=Proxy；functions=require, toJSON。
...
pub enum ProxyState { Active, Inactive, Deleted }
...
impl ProxyModel {
    pub const SOURCE_PATH: &'static str = "server/model/proxy.js";

    pub fn new(id: i64, name: impl Into<String>) -> Self {
        logging::debug("model.proxy", "new", format!("id={id}"));
        Self { id, name: name.into(), state: ProxyState::Active, created_date: None, updated_date: None, fields: Vec::new() }
    }

    pub fn validate(&self) -> Result<(), String> {
        // 參考系統 BeanModel 寫入前會檢查必要欄位；Rust 版集中在 validate，避免空名稱資料流入 socket/router。
        logging::debug("model.proxy", "validate", &self.name);
        if self.name.trim().is_empty() { return Err("proxy name is required".to_string()); }
        Ok(())
    }

    pub fn set_field(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        let key = key.into();
        logging::debug("model.proxy", "set_field", &key);
        self.fields.push((key, value.into()));
        self
    }

    pub fn to_json_pairs(&self) -> Vec<(String, String)> {
        logging::debug("model.proxy", "to_json_pairs", format!("field_count={}", self.fields.len()));
...
pub fn proxy_run(model: &ProxyModel) -> Result<Vec<(String, String)>, String> {
    logging::debug("model.proxy", "run", "validate and serialize");
    model.validate()?;
